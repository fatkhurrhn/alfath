...
const games = [
    ...
    {
        id: 4,
        slug: "tebak-arti-surah",
        name: "Tebak Arti Surah",
        icon: "ri-translate-2",
        modes: [],
    },
    {
...
export default function GamesList() {
    ...
    const gamesWithModes = games.filter((g) => g.modes.length > 0);
    const gamesWithoutModes = games.filter((g) => g.modes.length === 0);

    return (
        <div className="pb-6 bg-[#fcfeff] min-h-screen px-1">
            {/* Game dengan mode */}
            <div className="space-y-4 mb-6 pt-2">
                {gamesWithModes.map((game) => {
                    ...
                    return (
                        <div
                            key={game.id}
                            className="bg-white rounded-xl shadow-sm border border-[#e5e9f0] overflow-hidden hover:shadow-md transition"
                        >
                            {/* Header */}
                            <div
                                className="flex items-center justify-between p-4 cursor-pointer"
                                onClick={() =>
                                    setExpanded(expanded === game.id ? null : game.id)
                                }
                            >
                                <div className="flex items-center gap-3">
                                    <div className="p-2 bg-[#fcfeff] border border-[#e5e9f0] rounded-lg">
                                        <i className={`${game.icon} text-lg text-[#355485]`}></i>
                                    </div>
                                    <h3 className="font-semibold text-[#355485]">{game.name}</h3>
                                </div>
                                <i
                                    className={`ri-arrow-down-s-line text-[#6d9bbc] transition-transform ${expanded === game.id ? "rotate-180" : ""
                                        }`}
                                ></i>
                            </div>
                            ...
                            {/* Content */}
                            <div
                                className={`transition-all duration-300 ${expanded === game.id
                                        ? "max-h-[800px] opacity-100"
                                        : "max-h-0 opacity-0"
                                    } overflow-hidden`}
                            >
                                <div className="p-4 space-y-6 border-t border-[#e5e9f0]">
                                    {/* Surah Mode */}
                                    {game.modes.includes("surah") && (
                                        <div>
                                            <h4 className="font-medium text-[#355485] mb-3 flex items-center gap-2 text-sm">
                                                <i className="ri-book-open-line text-[#6d9bbc]"></i>
                                                Pilih Surah
                                            </h4>

                                            {/* Search bar */}
                                            <div className="relative mb-3">
                                                <i className="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-[#6d9bbc]"></i>
                                                <input
                                                    type="text"
                                                    ...
                                                    className="w-full pl-10 pr-4 py-2 text-sm border border-[#e5e9f0] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#355485] bg-[#fcfeff]"
                                                />
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-2 max-h-60 overflow-y-auto pr-2 scrollbar-hide">
                                                {filteredSurahs
                                                    .filter(
                                                        (s) =>
                                                            !selectedLetter || s.name.startsWith(selectedLetter)
                                                    )
                                                    .map((s) => (
                                                        <Link
                                                            key={s.number}
                                                            to={`/game/${game.slug}/surah/${s.slug}`}
                                                            className="p-2.5 text-sm bg-[#fcfeff] rounded-lg hover:bg-[#f1f6fa] border border-[#e5e9f0] flex flex-col transition"
                                                        >
                                                            <span className="font-medium text-[#355485]">
                                                                {s.name}
                                                            </span>
                                                            <span className="text-xs text-[#6d9bbc] mt-1">
                                                                {s.verses} ayat
                                                            </span>
                                                        </Link>
                                                    ))}
                                                ...
                                                {filteredSurahs.length === 0 && (
                                                    <div className="col-span-2 text-center py-4">
                                                        <i className="ri-search-eye-line text-2xl text-[#6d9bbc] mb-2"></i>
                                    ...
                                    {game.modes.includes("juz") && (
                                        <div>
                                            <h4 className="font-medium text-[#355485] mb-3 flex items-center gap-2 text-sm">
                                                <i className="ri-file-list-2-line text-[#6d9bbc]"></i>
                                                Pilih Juz
                                            </h4>
                                            <div className="grid grid-cols-3 gap-2 max-h-40 overflow-y-auto pr-2 scrollbar-hide">
                                                {juzList.map((juz) => (
                                                    <Link
                                                        key={juz}
                                                        to={`/game/${game.slug}/juz/${juz}`}
                                                        className="px-2 py-2 text-sm text-center bg-[#fcfeff] rounded-lg hover:bg-[#f1f6fa] border border-[#e5e9f0] text-[#355485] transition"
                                                    >
                                                        Juz {juz}
                                                    </Link>
                                                ))}
            ...
            {/* Game tanpa mode */}
            <div>
                <h2 className="font-medium text-[#355485] mb-3 text-sm uppercase tracking-wide">
                    Game Lainnya
                </h2>
                <div className="grid grid-cols-2 gap-3">
                    {gamesWithoutModes.map((game) => (
                        <Link
                            key={game.id}
                            to={`/game/${game.slug}`}
                            className="bg-white p-4 rounded-xl shadow-sm border border-[#e5e9f0] hover:shadow-md flex flex-col items-center justify-center text-center h-32 transition">
                            <div className="p-2 bg-[#fcfeff] border border-[#e5e9f0] rounded-lg mb-2">
                                <i className={`${game.icon} text-lg text-[#355485]`}></i>
                            </div>
                            <h3 className="font-medium text-[#355485] text-sm">
                                {game.name}
                            </h3>
                            <p className="text-xs text-[#6d9bbc] mt-1">Mulai Game</p>
                        </Link>
                    ))}
                </div>
            </div>
