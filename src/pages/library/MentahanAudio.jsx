...
export default function MentahanAudio() {
    ...
    return (
        <div className="bg-white min-h-screen text-gray-800">
            ...
            <div className={`fixed top-0 left-0 w-full z-50 bg-white border-b border-gray-200 transition-transform duration-300 ${showHeader ? "translate-y-0" : "-translate-y-full"}`}>
                <div className="max-w-xl mx-auto flex items-center justify-between px-3 py-3">
                    <Link to="/" className="flex items-center font-semibold gap-2 text-[#355485] text-[15px]">
                        <i className="ri-arrow-left-line"></i> Mentahan Audio
                    </Link>
                    <Link to="/settings">
            ...
            <section className="max-w-full mx-auto px-2 pt-[70px] pb-2">
                {/* Category Filter - Horizontal Scroll for Mobile */}
                <div className="mb-3 overflow-x-auto pb-0">
                ...
                <div className="space-y-2">
                    {loading ? (
                        ...
                        filteredAudios.map((audio) => (
                            <div
                                key={audio.id}
                                className="bg-white border border-gray-200 rounded-[13px] p-3 shadow-sm hover:shadow-md transition-shadow"
                            >
                                <div className="flex items-center justify-between flex-wrap gap-2 sm:gap-4">
                                    ...
                                    <div className="flex items-center gap-2 flex-shrink-0">
                                        <button
                                            ...
                                            className="w-[30px] h-[30px] rounded-full bg-gray-600 text-white hover:bg-gray-700 transition-colors flex items-center justify-center"
                                            aria-label={currentlyPlaying === audio.id ? 'Pause' : 'Play'}
                                        >
                                            {currentlyPlaying === audio.id ? (
                                                <i className="ri-pause-fill text-[14px]"></i>
                                            ) : (
                                                <i className="ri-play-fill text-[14px]"></i>
                                            )}
                                        </button>
                                        <button
